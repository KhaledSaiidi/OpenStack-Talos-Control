...
# Production bootstrapper for the Talos cluster Terraform stack.
# Handles terraform lifecycle (init/plan/apply/destroy) and outputs cluster connection hints.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
TF_DIR="${REPO_ROOT}/terraform/talos-cluster-bootstrap"
LOG_DIR="${REPO_ROOT}/logs"
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/bootstrap-talos-$(date +%Y%m%d-%H%M%S).log"
exec > >(tee -a "${LOG_FILE}")
exec 2>&1
...
Bootstraps the Talos Kubernetes management stack via Terraform.
...
summarize_outputs() {
  log "INFO" "Terraform outputs (Talos nodes and network):"
  terraform output -json | jq '{masters: .master_nodes.value, workers: .worker_nodes.value, network: .network_name.value}'
  log "INFO" "Talos inventory file: ${TF_DIR}/ansible_inventory.yaml (generated by Terraform)"
}
...
main() {
  ...
  popd >/dev/null
  log "INFO" "Completed '${ACTION}' for Talos stack."
}
